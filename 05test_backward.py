...
from paddlepaddle.cvnets.layers import AdjustBatchNormMomentum as adjust_paddle
...
from reference.cvnets.layers import AdjustBatchNormMomentum as adjust_torch
...
def test_backward():
    ...
    adjust_norm_mom_paddle = adjust_paddle(opts=opts)

    opt_torch = build_optimizer_ref(torch_model, opts)
    lr_scheduler_torch = build_scheduler_ref(opts)
    adjust_norm_mom_torch = adjust_torch(opts=opts)
