...
from .transformer_paddle import TransformerEncoder, LinearAttnFFN
...
class MobileViTBlock(BaseModule):
    ...
    def forward_spatial(self, x: Tensor) -> Tensor:
        ...
        if self.fusion is not None:
            fm = self.fusion(paddle.concat((res, fm), axis=1))
        return fm
    ...
    def profile_module(
        ...
        if self.fusion is not None:
            out, p, m = module_profile(
                module=self.fusion, x=paddle.concat((out, res), axis=1)
            )
            params += p
            macs += m
...
class MobileViTBlockv2(BaseModule):
    ...
    def _compute_unfolding_weights(self) -> Tensor:
        # [P_h * P_w, P_h * P_w]
        weights = paddle.eye(self.patch_h * self.patch_w, dtype='float32')
...
class MobileViTBlockv3(BaseModule):
    ...
    def folding_pytorch(self, patches: Tensor, output_size: Tuple[int, int]) -> Tensor:
        ...
        # [B, C, P, N]
        patches = paddle.reshape(patches,[batch_size, in_dim * patch_size, n_patches])
    ...
    def profile_module(
        ...
        out, p, m = module_profile(
            module=self.conv_proj, x=paddle.concat((fm, out), axis=1))
        params += p
        macs += m
