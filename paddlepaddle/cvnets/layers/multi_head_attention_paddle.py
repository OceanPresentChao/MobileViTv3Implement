...
class MultiHeadAttention(BaseLayer):
    ...
    def forward_tracing(self, x_q: Tensor, x_kv: Optional[Tensor] = None) -> Tensor:
        if x_kv is None:
            ...
            # # [N, P, 3C] --> # [N, P, C] x 3
            query, key, value = paddle.chunk(qkv, chunks=3, axis=-1)
        else:
            # [N, P, C]
            query = F.linear(
                x_q,
            ...
            key, value = paddle.chunk(kv, chunks=2, axis=-1)

        query = query * self.scaling

        # [N, P, C] --> [N, P, c] x h, where C = c * h
        query = paddle.chunk(query, chunks=self.num_heads, axis=-1)
        value = paddle.chunk(value, chunks=self.num_heads, axis=-1)
        key = paddle.chunk(key, chunks=self.num_heads, axis=-1)

        wt_out = []
        for h in range(self.num_heads):
            attn_h = paddle.matmul(query[h], key[h].transpose(-1, -2))
            attn_h = self.softmax(attn_h)
            attn_h = self.attn_dropout(attn_h)
            out_h = paddle.matmul(attn_h, value[h])
            wt_out.append(out_h)

        wt_out = paddle.concat(wt_out, axis=-1)
