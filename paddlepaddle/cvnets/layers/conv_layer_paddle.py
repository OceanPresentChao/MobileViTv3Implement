...
class ConvLayer(BaseLayer):
    ...
    def profile_module(self, input: Tensor) -> Tuple[Tensor, float, float]:
        ...
        output = paddle.zeros(
            shape=(b, self.out_channels, out_h, out_w),
            dtype=input.dtype,
        )
        # print(macs)
...
class ConvLayer3d(BaseLayer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        padding = tuple([int((kernel_size[i] - 1) / 2) * dilation[i]
                        for i in range(3)])
    ...
    def profile_module(self, input: Tensor) -> Tuple[Tensor, float, float]:
        ...
        output = paddle.zeros(
            shape=(b, self.out_channels, out_d, out_h, out_w),
            dtype=input.dtype,
            device=input.device,
        )
        return output, params, macs
