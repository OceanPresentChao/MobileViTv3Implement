...
from .conv_layer_paddle import ConvLayer
...
class LinearSelfAttention(BaseLayer):
    ...
    def visualize_context_scores(context_scores):
        ...
        # [1, 1, P, N] --> [1, P, h, w]
        context_scores = context_scores.reshape(
            1, num_pixels, patch_h, patch_w)
        # Fold context scores [1, P, h, w] using pixel shuffle to obtain [1, 1, H, W]
        context_map = F.pixel_shuffle(
            context_scores, upscale_factor=up_scale_factor)
        # [1, 1, H, W] --> [H, W]
        context_map = context_map.squeeze()

        # For ease of visualization, we do min-max normalization
        min_val = paddle.min(context_map)
        max_val = paddle.max(context_map)
        ...
        try:
            ...
            colored_context_map = cv2.applyColorMap(
                context_map, cv2.COLORMAP_JET)
            ...
            f_name = "{}/h_{}_w_{}_index_".format(
                res_dir_name, patch_h, patch_w)

            files_cmap = glob(
                "{}/h_{}_w_{}_index_*.png".format(res_dir_name,
                                                  patch_h, patch_w)
            )
            idx = len(files_cmap)
            f_name += str(idx)
    ...
    def _forward_self_attn(self, x: Tensor, *args, **kwargs) -> Tensor:
        ...
        query, key, value = paddle.split(
            qkv, num_or_sections=[1, self.embed_dim, self.embed_dim], axis=1
        )
    ...
    def _forward_cross_attn(
        ...
        query, key = paddle.split(qk, num_or_sections=[
            1, self.embed_dim], axis=1)
        # [B, C, P, N] --> [B, d, P, N]
        value = F.conv2d(
            x,
            weight=self.qkv_proj.block.conv.weight[self.embed_dim + 1:, ...],
