...
from paddlepaddle.utils.download_utils import get_local_path
from paddlepaddle.utils import logger
from paddlepaddle.utils.common_utils import check_frozen_norm_layer
from paddlepaddle.utils.ddp_utils import is_master, is_start_rank_node
...
def register_cls_models(name):
    def register_model_class(cls):
        ...
        if not issubclass(cls, BaseEncoder):
            raise ValueError(
                "Model ({}: {}) must extend BaseEncoder".format(
                    name, cls.__name__)
            )
...
def build_classification_model(opts, *args, **kwargs):
    ...
    if model_name in CLS_MODEL_REGISTRY:
        cls_act_fn = getattr(
            opts, "model.classification.activation.name", None)
        if cls_act_fn is not None:
            ...
            gen_act_neg_slope = getattr(
                opts, "model.activation.neg_slope", 0.1)
    ...
    if finetune_task:
        ...
        if pretrained is not None:
            ...
            model = load_pretrained_model(
                model=model, wt_loc=pretrained, is_master_node=is_start_rank_node(
                    opts)
            )
    ...
    freeze_norm_layers = getattr(
        opts, "model.classification.freeze_batch_norm", False)
...
for file in os.listdir(models_dir):
    ...
    if (
        ...
        module = importlib.import_module(
            "paddlepaddle.cvnets.models.classification." + model_name)
