...
from ...misc.init_utils_paddle import initialize_weights, initialize_fc_layer
...
class BaseEncoder(nn.Layer):
    ...
    def reset_parameters(self, opts):
        ...
        initialize_weights(opts=opts, modules=self.sublayers())
    ...
    def extract_end_points_l4(self, x: paddle.Tensor, *args, **kwargs) -> Dict[str, paddle.Tensor]:
        return self.extract_end_points_all(x, use_l5=False)

    def extract_features(self, x: paddle.Tensor, *args, **kwargs) -> paddle.Tensor:
        x = self.conv_1(x)
        x = self.layer_1(x)
        x = self.layer_2(x)
        x = self.layer_3(x)
    ...
    ) -> Tuple[paddle.Tensor, float, float]:
    ...
    def profile_model(
        self, input: paddle.Tensor, is_classification: Optional[bool] = True, *args, **kwargs
    ) -> Tuple[Union[paddle.Tensor, Dict[str, paddle.Tensor]], float, float]:
        """
        ...
        if is_classification:
            logger.log(
                "Model statistics for an input of size {}".format(input.shape))
        ...
        if is_classification:
            ...
            print(
                "{:<20} = {:>8.3f} M".format(
                    "Overall parameters", overall_params / 1e6)
            )
